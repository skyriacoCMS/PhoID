...
void EfALLweighted(){
  ...
  TH1F *EffPTM    = new TH1F("EffPTM","Signal Eff vs PT",100,0,200);
  TH1F *EffETAM   = new TH1F("EffETAM","Signal Eff vs Eta",100,0,5);

  TH1F *EffNVTXL  = new TH1F("EffNVTXL","Signal Eff vs NVTX",100,0,100);
  TH1F *EffPTL    = new TH1F("EffPTL","Signal Eff vs PT",100,0,200);
  TH1F *EffETAL  = new TH1F("EffETAL","Signal Eff vs Eta",100,0,5);

  TH1F *EffNVTXT  = new TH1F("EffNVTXT","Signal Eff vs NVTX",100,0,100);
  TH1F *EffPTT    = new TH1F("EffPTT","Signal Eff vs PT",100,0,200);
  TH1F *EffETAT   = new TH1F("EffETAT","Signal Eff vs Eta",100,0,5);

  TH1F *BckNVTX0 = new TH1F("BckNVTX0","Bck Rej vs NVTX 0",100,0,100);
  TH1F *BckPT0   = new TH1F("BckPT0","Bck Rej vs PT 0",100,0,200);
  TH1F *BckETA0  = new TH1F("BckETA0","Bck Rej vs Eta",100,0,5);

  TH1F *BckNVTXL = new TH1F("BckNVTXL","Bck Rej vs NVTX",100,0,100);
  TH1F *BckPTL   = new TH1F("BckPTL","Bck Rej vs PT",100,0,200);
  TH1F *BckETAL  = new TH1F("BckETAL","Bck Rej vs Eta",100,0,5);

  TH1F *BckNVTXM = new TH1F("BckNVTXM","Bck Rej vs NVTX",100,0,100);
  TH1F *BckPTM   = new TH1F("BckPTM","Bck Rej vs PT",100,0,200);
  TH1F *BckETAM  = new TH1F("BckETAM","Bck Rej vs Eta",100,0,5);

  TH1F *BckNVTXT = new TH1F("BckNVTXT","Bck Rej vs NVTX",100,0,100);
  TH1F *BckPTT   = new TH1F("BckPTT","Bck Rej vs PT",100,0,200);
  TH1F *BckETAT  = new TH1F("BckETAT","Bck Rej vs Eta",100,0,5);


  TH1F *EffPTs  = new TH1F("EffPTs","Signal Eff sIEIE vs PT ",100,0,200);
  TH1F *EffPTt   = new TH1F("EffPTt","Signal Eff hOE   vs PT ",100,0,200);
  TH1F *EffPTp   = new TH1F("EffPTp","Signal Eff ISOp  vs PT ",100,0,200);
  TH1F *EffPTc   = new TH1F("EffPTc","Signal Eff ISOc  vs PT ",100,0,200);
  TH1F *EffPTn   = new TH1F("EffPTn","Signal Eff ISON  vs PT ",100,0,200);


  TH1F *EffPTc1   = new TH1F("EffPTc1","Signal Eff vs PT c1",100,0,200);
  TH1F *EffPTc2   = new TH1F("EffPTc2","Signal Eff vs PT c2",100,0,200);
  TH1F *EffPTc3   = new TH1F("EffPTc3","Signal Eff vs PT c3",100,0,200);
  TH1F *EffPTc4   = new TH1F("EffPTc4","Signal Eff vs PT c4",100,0,200);
  TH1F *EffPTc5   = new TH1F("EffPTc5","Signal Eff vs PT c5",100,0,200);

  TH1F *Ptweight   = new TH1F("Ptweight"," weight pt for both samples",100,0,200);
  TH1F *Ptw   = new TH1F("Ptw"," weight pt for both samples",100,0,200);
  TH1F *PtweightB   = new TH1F("PtweightB"," weight pt for both samples BACK",100,0,200);
  TH1F *PtwB   = new TH1F("PtwB"," weight pt for both samples BACK",100,0,200);

  TH2D *SiePt   = new TH2D("SiePt","Sieie vs PT ",100,0,200,1000,0,0.1);
  TH2D *ToEPt   = new TH2D("ToEPt","Toe vs PT   ",100,0,200,5000,0,5);
  TH2D *IsoPPt   = new TH2D("IsoPPt","IsoP vs PT ",100,0,200,100000,0,100);
  TH2D *IsoCPt   = new TH2D("IsoCPt","IsoC vs PT ",100,0,200,100000,0,100);
  TH2D *IsoNPt   = new TH2D("IsoNPt","IsoN vs PT ",100,0,200,100000,0,100);



  Input file:
  TFile *f400 = new TFile( "../../../CutTMVATrees_EndCap.root");
  ...
  double slopeP = 0.0059; 
  double slopeN = 0.0118;
  ...
  for(int i  = 0 ; i < t_S->GetEntries(); i++){
    ...
    if((iso_P < IsoPcutL+slopeP*Ppt)&&(iso_N < IsoNcutL+slopeN*Ppt)&&(iso_C < IsoCcutL)&&(to_e<HoEcutL)&&(Sie_ie < SieiecutL )){
    ...
    if((iso_P < IsoPcutM+slopeP*Ppt)&&(iso_N < IsoNcutM+slopeN*Ppt)&&(iso_C < IsoCcutM)&&(to_e<HoEcutM)&&(Sie_ie < SieiecutM )){
    ...
    if((iso_P < IsoPcutT + slopeP*Ppt)&&(iso_N < IsoNcutT + slopeN*Ppt)&&(iso_C < IsoCcutT)&&(to_e<HoEcutT)&&(Sie_ie < SieiecutT )){
  ...
  TFile *f1_0 = new TFile("EffStudies.root","RECREATE");
  f1_0->cd();
